...
export const validateExpenseInput = async (req: Request, res: Response, next: NextFunction) => {
    ...
    await body('name')
        .notEmpty().withMessage('El nombre del gasto no puede estar vacío').run(req)
    await body('amount')
        .notEmpty().withMessage('La cantidad del gasto no puede estar vacía')
        .isNumeric().withMessage('Cantidad no válida')
        .custom(value => value > 0).withMessage('La cantidad del gasto debe ser mayor que 0').run(req)
